...
export default {
  ...
  methods: {
    ...
    initRightEvent() {
      ...
      handler.setInputAction(movement => {
        ...
        if (this.rightValue === "annotate") {
          this.annotate(movement, feature);// 标签显示信息
        } else if (this.rightValue === "properties") {
          this.printProperties(movement, feature);// 控制台打印属性信息
        } else if (this.rightValue === "zoom") {
          this.zoom(movement, feature);// 定位到当前feature
        }
      }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);
      // 鼠标中建单击事件
      handler.setInputAction(movement => {//movement移动
        // 获取选中的feature
        const feature = this.viewer.scene.pick(movement.position);
        if (!Cesium.defined(feature)) {
          return;
    ...
    annotate(movement) {
      // 如果支持选中位置
      if (this.viewer.scene.pickPositionSupported) {//Supported支持
    ...
    zoom(movement, feature) {
      ...
      const transform = Cesium.Transforms.eastNorthUpToFixedFrame(position);//本地坐标在世界坐标中的变换矩阵
      Cesium.Matrix4.multiplyByPoint(transform, offset, position);//一个4X4矩阵与一个点（平移向量）的叉乘
      camera.flyTo({
        ...
        easingFunction: Cesium.EasingFunction.QUADRATIC_OUT//控制在飞行过程中如何插值时间
      });
    },
