<template>
  <div style="display: flex">
    ...
    <div class="control">
      <button @click="linkActive">联动</button>
      <button @click="linkDeactive">关闭</button>
    </div>
  </div>
</template>
...
import Cesium2DLink3D from "../utils/control/control_2DLink3D";
export default {
  components: {},
  ...
  mounted() {
    ...
    this.viewer.camera.setView({
      destination: Cesium.Cartesian3.fromDegrees(116.40125, 39.899, 3995000.0),
    });
    this.linkUtil = new Cesium2DLink3D();
  },

  methods: {
    linkActive() {
      if (this.linkUtil.isActive) return;
      this.linkUtil.active(this.viewer);
    },
    linkDeactive() {
      this.linkUtil.deactive();
    },
  },
};
</script>
