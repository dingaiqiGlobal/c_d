...
export default {
  ...
  mounted() {
    ...
    handler.setInputAction((movement) => {
      ...
      this.addMarkBillboard(position, "images/icon/BIM/mark_Billboard.png", "墙体检查");
    }, Cesium.ScreenSpaceEventType.LEFT_CLICK);
  },

  methods: {
    addMarkBillboard(position, image, text) {
      this.viewer.entities.add({
        position: position,
        billboard: {
          image: image,
          pixelOffset: new Cesium.Cartesian2(45, -5),
          disableDepthTestDistance: Number.POSITIVE_INFINITY,
        },
        label: {
          text: text,
          font: "small-caps 14px sans-serif",
          pixelOffset: new Cesium.Cartesian2(65, -10),
          disableDepthTestDistance: Number.POSITIVE_INFINITY,
        },
      });
    },
