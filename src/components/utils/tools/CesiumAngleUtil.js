...
export class CesiumAngleUtil {
    ...
    cartesianToLngLat(cartesian) {
        const latlng = this.viewer.scene.globe.ellipsoid.cartesianToCartographic(
            cartesian
        );
    ...
    getCatesian3FromPX(px) {
        let cartesian;
        //在模型上提取坐标
        var pickobject = this.viewer.scene.pick(px); //取模型
        if (this.viewer.scene.pickPositionSupported && Cesium.defined(pickobject)) {   //!scene.pickPositionSupported : 不支持深度拾取,无法进行鼠标交互绘制
            cartesian = this.viewer.scene.pickPosition(px);
            if (cartesian) {
                const pgeo = this.viewer.scene.globe.ellipsoid.cartesianToCartographic(cartesian);
                if (pgeo.height > 0)
                    return cartesian;
            }
        }
        ...
        //提取鼠标点的地理坐标
        const pickRay = this.viewer.scene.camera.getPickRay(px);
