...
export default class Cesium2DLink3D {
  ...
  init2DMap() {
    ...
    this.olMap = new Map({
      ...
      target: this.mapId,
      view: new View({
        center: fromLonLat([116.40125, 39.899]),
        zoom: 8,
  ...
  changeCenterListener() {
    if (this.isIn2DMapFlag) {
      ...
      const boundsTansform = olProj.transformExtent(bounds, 'EPSG:3857', 'EPSG:4326')
  ...
  getViewCameraRectrange() {
    ...
    if (!this.isIn2DMapFlag) {
      if (north > 87 && south < -87) {
        ...
        this.olMap.getView().setCenter(olProj.transform([center.lon, center.lat], 'EPSG:4326', 'EPSG:3857'))
      } else {
        this.olMap.getView().fit(olProj.transformExtent([west, south, east, north], 'EPSG:4326', 'EPSG:3857'))
      }
    }
  }
